---
title: '10 Things For a Better Scientific Paper from LaTeX to the Command Line'
...
To help people **practice** these commands we have a [Jupyter Lab]() instance
...
can start this environment [here](link). 
...
# Flavors and Programs: tex, latex,  pdflatex, etc ... basic gist

A "LaTeX document" is short hand for a plain text document written with markup like `\textbf{this is bold}` for bold text or `$ \frac{x^2}{\log(x)}$` for math. Once you have a plain text document with markup, you then process it using a set of programs.

In general, the first and most basic program that parses such markup is called `tex' but, in current daily use, you will mostly find yourself using `pdflatex` or `xelatex` or maybe `lualatex`.
...
Notice also:
 ...
 - `pdflatex` (or `xelatex` or `lualatex`) takes several passes
 - Tools like `latexmk` or `latexrun` automate this
...
- DON’T use \FloatBarrier and other tricks to for spacing
